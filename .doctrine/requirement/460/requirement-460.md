# REQ-460: No failure, staleness, replay, or cleanup path force-updates canonical state, auto-resolves content, resumes a frozen capsule, or automatically destroys unresolved work.

## Statement

<!-- The sister TOML's `description` field is the primary, normative statement.
     Prose here may elaborate, expand upon, or disambiguate it — never
     duplicate it. -->

## Rationale

<!-- Why it must hold — the force behind it, not the implementation. -->
