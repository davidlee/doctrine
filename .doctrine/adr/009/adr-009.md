...
## Amends ADR-003 (APPLIED to `adr-003.md` on acceptance, 2026-06-09)
...
- No slice-level `review` state exists; per-phase review (ADR-003 §6) lives in
  ...
  `audit` and `close`.
- On acceptance + the execution-phase boot edit, `.doctrine/state/boot.md`
  Core-process prose **will name** `… audit → reconcile → close` (the state
  exists); the routing-table *skill* row stays unchanged until the `/reconcile`
  skill lands — pointing it at a deferred skill would be shipped-not-reachable
  (F2/F14). Until then, reconcile-entry is manual discipline.
