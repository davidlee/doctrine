...
`checkout_state_id` reproduces forgettable's `forget.checkout.v1` normalizer
**byte-for-byte**, proven by a conformance golden vector (SL-007 **VT-3**).
Symlink hashing is **deferred in forgettable itself**, so the frame normalizer
...
breaking the forgettable conformance contract** (SL-007 VT-3 golden vector must
stay byte-identical for symlink-free trees).

**DECIDED (design D1): direction 3 — upstream-first.** forgettable un-defers m12
(the gate is a verbatim copy of `forgettable/src/git_context.rs:727`); doctrine
then re-syncs the mirrored gate + the VT-3 golden vector. **Slice is `blocked`
on the forgettable change.** Directions 1 (record soft-fail) and 2 (doctrine-local
...
3. **Un-defer symlink encoding** — define a conformant symlink hashing in
   `forget.checkout.v1` upstream in forgettable, then mirror it byte-for-byte
...
- **No locally-invented symlink hash.** Hashing the symlink target text *only in
  ...
  golden vector. Out of scope unless forgettable un-defers (direction 3).
...
- **Q2 — is the symlink gate even semantically right for `repo_id`?** `repo_id`
  ...
  the tree.
- **A1 —** forgettable's `forget.checkout.v1` still defines no symlink encoding
  (assumed; verify against `<forgettable>/.spec-driver` before direction 3).
...
- Port `doc/memories/{customizable-governance-surface, doctrine-just-module,
  engine-identity-and-claim-seam, forgettable-event-store-backend-contract}` into
  the memory store once `record` works.
- Possible upstream slice: un-defer symlink encoding in forgettable
