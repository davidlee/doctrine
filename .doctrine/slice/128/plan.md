...
test pins the two together (I1). Deliberately introduces **no** live read of
`doc.dispatch`, so the existing `expect(dead_code)` stays fulfilled and the build
stays green — the dead-code removal is held until its real consumer lands.
...
is replaced by behaviour tests (codex-F4). The `deliver-to` verb is the thin read
that PHASE-04's prose will call.
...
literal, not the fork-base auto-resolver. The step-3a TODO that motivated IMP-124
is removed.
