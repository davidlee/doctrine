> **SUPERSEDED — archive.** This file is the round-1→7 reasoning trail, retired by
> the 8th-inquisition redesign. The live design is `design.md`; the per-charge
> dispositions live in `inquisition-1.md`…`inquisition-7.md`. Preserved unedited
> (incl. mid-flight partial round-7 ο edits, now abandoned) for provenance only —
> **do not treat as canon.** Read `design.md`.
...
- **DC-1 (seam boundary, per-harness spawn — Charge XIII).** The
  ...
  - **codex/pi:** `doctrine worktree fork` creates the worktree, stamps the marker,
    ...
    subprocess (`codex exec` / pi) **with its cwd bound to the fork dir** (else the
    worker process edits the coordination root the orchestrator ran `fork` from —
    round-7 Charge ν) and that env (+ `DOCTRINE_WORKER`, + bwrap). The cwd-bind is a
    **spawn-shell mechanism** (DC-1: how-spawned is harness-shaped prose), not a
...
```
`$fork_env` is the stdout per-wt env block; status went to stderr. **`env -C "$D"`
binds the worker process's cwd to the fork (round-7 Charge ν)** — without it `codex
exec` inherits the orchestrator's cwd (the coordination root it ran `fork` from) and
the worker's *source* edits land on the trusted branch, bypassing `import`, the
`.doctrine/` belt, and branch-point discipline; the `DOCTRINE_WORKER` guard catches
only doctrine-mediated writes, never a raw editor write to cwd. `"$D"` is the
orchestrator's own validated `--dir` (already `fork`-checked for existence/uniqueness)
— machine data, quoted. The cwd-bind lives in the **spawn shell, not the prompt**
(DC-1). If `env -C` is unavailable, the portable equivalent is a subshell
`( cd "$D" && exec env DOCTRINE_WORKER=1 $fork_env codex exec … )` — same effect,
orchestrator cwd untouched. When D6 lands the bwrap wrap (`bwrap <profile> --chdir
"$D" env … codex exec …`) **bwrap's `--chdir "$D"` is the confined equivalent of the
cwd-bind** — both put the worker inside the fork; confinement is a harness/OS
concession, DC-1 keeps it in prose. **claude is unaffected:** the `Agent` tool with
`isolation:worktree` runs the worker *inside* the worktree it created, so its cwd is
the fork by construction — no spawn-shell cwd-bind to add (the ν hazard is
subprocess-only, like D5/D6).
...
worktree handle to the orchestrator**, so it cannot *pre-bind* an arming to the
*specific* WorktreeCreate it intends; under a concurrent batch a second
`isolation:worktree` fire would read the armed sentinel and brand the **wrong** tree
(the intended worker then unmarked → fail-open). (The **hook itself**, by contrast,
*does* receive `{session_id, name, cwd}` on stdin — round-7 Charge ο turns the
worktree `name` into a **stamp-correlation key** so a *stale* late Agent cannot
mis-stamp; the single-slot here still governs *intentional* concurrency.) So claude marker-stamping requires **one armed spawn in
...
- **Removers (Q2) — three, mirroring the marker's:** (a) the hook's **disarm-on-stamp**
  ...
  self-bricked) — the **load-bearing abandoned-arm recovery** (round-7 Charge ο) — the verb a
  compliant orchestrator whose Agent died calls to clear the slot **without filesystem
  surgery** (the Charge II cure, applied to the sentinel); **it clears a live *or*
  expired lease** (the operator's deliberate act carries the quiescence assertion the
  CLI cannot prove); (c) **lease expiry** — `--arm` writes a staleness bound (a lease deadline
...
**gc is an idempotent state machine over its own partial states (round-7 Charge ξ —
the cleanup owner must resume the cleanup it started, not strand its debris).** μ
closes the crash window *downstream in `land`*; ξ closes it *in `gc` itself* — gc that
crashed between any two destructive steps **completes on rerun or names the leftover**,
never leaving the operator to infer state. The model is the **presence** of the three
reapables `{branch B, linked worktree W, target dir T = wt/<branch>}`, gathered impurely
and classified purely (`classify_gc(state) -> GcPlan`: which steps remain, and whether
the landed-gate must run), holding the pure/imperative wall:
- **Gate runs only while the branch lives.** The landed-oracle (patch-id / ancestry)
  and `--superseded-head` both **require B** (`git cherry`/`--is-ancestor`/head-compare
  are over the branch). When **B present**, run the gate exactly as today (refuse unless
  certified); the oracle is **monotone and re-runnable** (a landed fork stays landed —
  round-5 acquittal: ancestry-leg monotonicity under an advancing HEAD), so a rerun
  after a mid-sequence crash re-certifies and resumes.
- **Branch-gone ⇒ the deletion *is* the certificate.** A fork branch is deleted **only
  through `branch -D` after this gate passed** (the funnel branch is never a git-ancestor;
  `-d` always refuses — D4), so once **B is absent** the fork was **already certified**
  by the run that deleted it. The remaining debris is post-certification and is reaped
  **without** re-running a gate that no longer has its operand.
- **The remaining debris is git-impossible-to-be-dangerous or purely derived.** Two
  cases exhaust branch-gone: **(a) W gone too** (gc's own ordering removes W in step 1
  *before* deleting B in step 2 — so gc's own crash never produces branch-gone-with-W);
  **(b) "branch gone, live linked worktree present" is git-impossible** — `git branch -D`
  **refuses a branch checked out in a live worktree** (the same fact μ rests on), so no
  external action can strand a *linked* worktree under an absent branch. gc therefore
  **never force-removes a worktree whose branch vanished**. The only branch-gone residue
  is **T**, the `wt/<branch>` **target dir** — derived build cache (`CARGO_TARGET_DIR`,
  D5), **never the only copy of anything**, and its path is a **pure function of the
  branch *name*** (`target_dir_for_branch`), so a rerun derives and reaps it **from
  `--fork <branch>` alone**, no git facts, no oracle. This is exactly the
  "branch-deletion-succeeded-but-target-reap-failed" case: the orphan T is reapable by
  name with B long gone.
- **Skip completed steps; reaping an absent thing is a no-op.** `worktree remove` on an
  already-gone worktree, `branch -D` on an already-gone branch, `rm` on an already-gone
  T — each is a silent skip, so a rerun is safe at any interruption point.
- **Every destructive step is Charge-VIII-honest on failure.** worktree-remove fail →
  name W's dir + B, exit non-zero; `branch -D` fail → name B, exit non-zero; target-reap
  fail → name T's path, exit non-zero. The rerun then resumes from the named leftover.
  A `git worktree prune` of a stale administrative entry (dir removed out-of-band, git's
  worktree list still lists it) is folded into the worktree leg.
- **The ultimate backstop** is the storage model: B's deletion is durable git; W and T
  are gitignored/disposable runtime+derived state the operator can `rm` as a last resort
  — gc names them, so recovery never requires guesswork.
...
**`--chdir`s into the worktree** (round-7 Charge ν) — the confined equivalent of the
non-bwrap `env -C "$D"` cwd-bind (D1), so the worker starts *inside* the fork even
under confinement; with main ro-bound, a stray cwd-relative write is doubly contained. **The profile also
