...
fresh-as-of: 2026-09-20 · **closed `done`**. `/reconcile` discharged `RV-372`'s brief into `design.md` and `slice-246.md` (eleven edits, zero REVs) — the outcome is `RV-372` § *Reconciliation Outcome*, not restated here. `CHR-073` discharged before the landing (five memories corrected and re-attested, seven authored sites repaired); its overflow is `CHR-076`. Landed by fast-forward onto `edge` and `main` at `0de338dce` — not squashed, so `mem.pattern.platform.never-export-git-dir-to-a-test-run` survives. Open and carried: `CHR-074`, `CHR-076`, `IMP-465`, `ISS-463`–`ISS-469`.
