...
src/worktree/
  ...
  shared.rs     cross-machine helpers: resolve_common_dir, is_linked_worktree,
                verify_sibling_worktree, primary_worktree, enumerate_candidates,
                resolve_commit, matches (branch ref-eq)
  ...
  provision.rs  run_provision, run_check_allowlist
  import.rs     Apply/Refusal/classify_import/gather_tree_clean/run_import
  ...
  subagent.rs   Stamp/StampRefusal/classify_stamp/SubagentPayload/cwd_shares_repo/
                ...
                classify_worker_verify/run_verify_worker
...
`mod.rs` must `pub(crate) use` the 9 externally-consumed symbols (8 caller files:
`main.rs`, `boot.rs`, `review.rs`, `commands/guard.rs`, `commands/cli.rs`,
`memory.rs`, `git.rs`, `dispatch.rs`):

`WorktreeCommand`, `dispatch`, `DISPATCH_WORKER_AGENT_TYPE`, `DUAL_CAUSE`,
`is_linked_worktree`, `resolve_mode`, `env_worker_set`, `gather_tree_clean`,
`coordinate`.

Miss one → caller breaks at `cargo build` (the re-export proof).

## Visibility

The only non-mechanical change: helpers currently file-private (`fn`, no vis) but
used by multiple machines once split — `resolve_common_dir`,
`verify_sibling_worktree`, `primary_worktree` (and any other discovered during the
move) — widen to **`pub(super)`** (visible within the `worktree` module only,
**not** re-exported). Single-machine private helpers stay private in their file.
Rule: widen to `pub(super)`, never `pub(crate)`, for internal helpers — minimal
surface, no external leak.
...
_(pending — internal hostile pass next, then optional `/inquisition` or external.)_
