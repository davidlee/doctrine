...
and `governance::GovKind`; replace all ~93 production `format!` sites that hand-roll
...
Sub-kinds (DESIGN_KIND, PLAN_KIND, NOTES_KIND) get `stem: ""`. A
...
~4 sites using `g.stem` become `g.kind.stem`. The 4 GovKind constructors
(ADR_KIND, POLICY_KIND, STANDARD_KIND, RFC_KIND) drop their `stem:` field.
...
1. **Add + seed:** Add `stem` to `Kind`, add `Ext`/`id_path`/`rel_path`, add `stem:`
   to all 23 Kind initializers. Compiles (KindRef/GovKind unchanged).
2. **Remove + replace:** Remove `KindRef::stem`, remove `GovKind::stem`,
   update all KINDS rows, update `g.stem` → `g.kind.stem`, replace all ~93 format!
