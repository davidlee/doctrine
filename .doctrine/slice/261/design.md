...
stateDiagram-v2
  ...
  Read --> RefusedLocked: stage = locked
  Read --> RefusedStale: --expect ≠ document
  Read --> RefusedMarkers: parse refuses\n(unknown / missing / malformed marker)
  Read --> Candidate: derive sections, invalidate, rebaseline
...
- **Refused, locked.** `adopt` never runs on a locked run (`DEC-279`). The
  remedy names both lawful paths: regress to `reviewing` for a run still
  governing execution, or edit directly without adopting at reconcile.
  The aligned test runs first, so an aligned locked run gets the no-op answer,
  which is also correct: there is nothing to adopt.
...
The two sides are the held section body and the document's section body. After
adoption, `materialise` reproduces `design.md` byte-identically, so the
per-section diff is exactly what materialising the run would change relative to
its prior snapshot. Rendered with the `similar` crate (`DEC-279`); unchanged
...
flowchart TD
  ...
  V[design adopt] --> A{aligned?}
  A -- yes --> N[print no-op, exit]
  A -- no --> B[ApplyRequest::bare<br/>Crossing::Adopt]
  O --> PL[apply_pipeline]
  B --> PL
  PL --> AD[admit]
  AD --> DV{Crossing}
  DV -- Ordinary --> DC[divergence check]
  DV -- Adopt --> AS[read authored_sections]
...
fn adopt_authored(
    ...
    if next.run.stage == Stage::Locked {
...
The locked check reads `next.run.stage`, which equals the prior stage at this
...
if !keys.iter().any(|row| row.key == name) {
    if let Some(retired) = retired_key(type_name, name) {
...
`retired_key` matches on `owner.name`, the Rust type name refusals already
cite. A pin asserts names are unique across the contract closure, so the match
cannot alias. The walk stays a
...
- Type names are unique across the contract closure.
...
- `REQ-434` (watermark guard) keeps its four criteria and gains one: *"The
  ...
  without it the entry read is the basis. `adopt` refuses on a locked run."*
...
band*) gains one line: `adopt` now refuses on a locked run, so the direct-edit
...
| `src/commands/design.rs` | `DesignCommand::Adopt(AdoptArgs)`; `run_adopt`; split `run_apply` into `parse_payload` + `apply_pipeline(…, &Crossing, Stop, …)`; `readopting` from `Crossing`; aligned short-circuit; report rendering (header, rows, unchanged, reordered, `--diff`); divergence and materialise read-back refusal text; `authored_sections` doc comment drops the caller-map reference |
| `src/design_run/run.rs` | `Crossing` enum; `apply` takes `&Crossing`; `adopt_authored(next, expect, derived)` without the map comparison, with the locked check |
...
| `src/design_run/contract_check.rs` | roster consult before `UnknownPayloadKey` |
...
- `adopt` on a locked run refuses with the regression remedy.
