...
- **Link** and **unlink**: if the uid exists only in shipped/, error: `"memory {uid} is a shipped corpus record — shipped/ is read-only (regenerated by 'doctrine memory sync'). Record a version in items/ first if you need to manage its relations."` If items/ has it, operate normally.
