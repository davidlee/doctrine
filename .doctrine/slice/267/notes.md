...
fresh-as-of: 2026-09-26 · CLOSE · 86f6cb566
...
- reconciled — design.md sec-1/sec-3/sec-7 (the authorised textual `src/**` departure) and sec-5 (the scope-field matcher class) corrected; the two falsified ledger rows annotated; `RV-395` `## Reconciliation Outcome` recorded (commit `86f6cb566`). `ADR-024` needed no revision, so no REV.
- minted at close — `IMP-487` (a delivery-freshness gate, the `F-13` gap); memory `mem.pattern.distribution.shipped-corpus-materialisation-freshness`.
- origins — `CHR-080` and `IMP-484` resolved/fixed (their sweeps are this slice's PHASE-04/05); `ISS-309` annotated (part 1 delivered by this slice, part 2 — the check — open).
...
- **mem.pattern.distribution.shipped-corpus-materialisation-freshness** — a skipped re-embed/`memory sync` leaves the materialised corpus stale while every gate stays green; verify with `cargo build && doctrine memory sync --dry-run` (`0 new, 0 changed` is the clean reading).
...
### Open
- `ISS-309` part 2 / `QUE-227` — the deferred shipped-corpus integrity gate; now carries **two** distinct checks: citations (content) and delivery freshness (`IMP-487`). The only durable defence against re-drift.
- `IMP-487` — a gate that refuses when an embedded corpus asset differs from its materialised delivery (`RV-395` `F-13`; a skipped `memory sync` is invisible to every gate).
- `ISS-215` — the three unreachable signposts (`mem.signpost.doctrine.{concept-map,rec,rfc}`); F-1 removed the broken citations, restoring the memories is its follow-through.
- `CHR-081` — consolidate the two local memories restating the grounding rule (out of scope; local-memory health is a non-goal corpus). `CHR-036` — distilling project-local memories.
- `install/glossary.md`'s `folder` column — undocumented convention, partly inconsistent; a faithful rebuild needs the engine's layout authority (`RV-395` `F-6` residual).
- The pre-audit "audit flag (PHASE-0N …)" leads are all dispositioned on `RV-395` — query the ledger rather than re-reading them here.
