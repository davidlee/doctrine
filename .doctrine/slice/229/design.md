...
> **Rule surfaced at audit (RV-306 F-3):** a new `src/` module *always* implies
> an ADR-001 `layering.toml` entry — the `architecture_layering` gate fails
> `Unclassified` without one. Any future design declaring a new module should
> declare that path up front rather than discovering it as an undeclared touch.
...
- **VA:** post-ritual, the installed copies exist and match
  ...
  SL-088.)
- **VH:** *partially satisfied at close (RV-306 F-2).* This slice's own dogfood
  round is the pre-design evidence and is real — `research/research.md` with its
  `raw/` thread output, cited throughout this design; it surfaced the
  pre-existing SL-055 storage convention and thereby overturned the scope's
  original state-tier+symlink wording into D1. The second leg — one *further*
  real slice driven through the round, observations to RFC-011 case-notes — is
  **deferred to CHR-048 step 4, not waived**: PHASE-03's hooks are authored and
  correct but not yet distributed (RV-306 F-1), so no agent can see them. R1 is
  consequently *untestable* until CHR-048 lands — the RFC-011 eval that would
  judge whether harder gating needs an ADR cannot run while no agent sees a hook.
...
- **R1** (hooks under-deliver without enforcement) — carried; mitigation is
  ...
  gating requires an ADR (D6). *At close (RV-306): not merely open but
  untestable — the mitigating eval cannot run until CHR-048 distributes the
  hooks. See § Verification alignment VH.*
