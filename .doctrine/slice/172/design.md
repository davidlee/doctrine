...
`max_upper` = max `upper` bound across **every** estimated entity in the scored
set, all kinds (global; kind-comparability already lives in `kind_weight`).
Because `est_cost_i = lower_i + β(upper_i−lower_i) ≤ upper_i ≤ max_upper`, a bare
item with `margin > 0` costs strictly more than every estimated item — the
inversion is impossible by construction, while value still multiplies through
(a high-value bare item can still outrank a low-value estimated one — proportional,
not a hard floor).
...
- **INV-2** `margin > 0` ⇒ every bare item's `est_cost` > every estimated item's
  `est_cost` (anchor dominance).
...
- **OQ-1 (resolved → reconcile)**: ADR-015 §1+§2+§4 amendment lands as a REV
  (ADR-013) authored at `/reconcile`, status stays `accepted` (amend, not
  supersede — refinement of an intact recent decision).
...
- **D4 — global, not per-kind anchor.** Cost is points, kind-independent;
  kind-comparability is already in `kind_weight`. Alt (per-kind): rejected — adds
  a keyed aggregate + per-kind empty fallback for no benefit.
- **D5 — amend, not supersede ADR-015.** Refinement of an intact decision.
...
- `bare_item_outranked_by_equal_value_estimated` (INV-2, ordering)
