...
Doctrine should separate project projection from semantic ownership, become
the canonical owner of behavioural contracts and locally enforced workflow
semantics, and derive activation mechanisms experimentally rather than presume
a route-only catalog. Dynamic delivery must be evaluated before privileged
activation metadata is reduced. The RFC accepts this direction while leaving
the storage, trust, protocol, taxonomy, and activator mechanisms to narrower
...
**C1 is now governed.** Its semantic-ownership root is ADR-019 (embedding /
publication / projection as independent asset policies); its **Contract A**
(minimal projection) landed as the SPEC-009 / PRD-006 revision under REV-028; its
**Contract B** (publication, curated library, federated search) is specified by
the now-active PRD-017 and SPEC-026. Those specs are the settled contract from
here; this RFC records the direction, not the mechanism.
...
"Behaviour definition" is an umbrella ownership term, not one monolithic
versioned object. Descendant specifications should separate at least:

```text
behaviour identity and composition contract
instruction fragments
selection policy
workflow predicates and transitions
resolver protocol
```

These layers may change and negotiate compatibility independently. A prose
revision should not automatically have the release or compatibility
consequences of changing transition semantics or resolver fields.
...
No slice should attempt to "implement RFC-021." Each stage proves one bounded
contract while preserving the current activation and enforcement surfaces not
under test. Trust needs early boundary analysis, but operational project
overrides need not block delivery experiments that use only framework-owned
definitions.
...
### Stage 3 — local workflow semantics and validation

Specify local states, validity predicates, named transitions, mutating-command
concurrency, and envelope refresh semantics. Prove atomic enforcement or
explicit optimistic-concurrency preconditions at the operations that mutate
Doctrine state without introducing a generic workflow interpreter or autonomous
executor.

### Stage 4 — activator experiments
...
### Stage 5 — trusted project overrides

Before project-supplied fragments become operationally authoritative, specify
the trust anchor, acceptance scope, change detection, re-acceptance policy,
override integrity, and approval UX. This stage may proceed earlier if an
earlier contract genuinely depends on project authority; framework-only
experiments must not quietly acquire that dependency.
...
Authored cases must be supplemented with prompts sampled from real Doctrine
sessions and replayed with identifying context normalized. Prioritize sessions
where activation was late, the agent needed reminding, `/consult` was missed,
routing was correct but irritating, trivial work triggered excess ceremony, or
compaction degraded an active process. These cases test ecological validity and
reduce benchmark cues that accidentally reveal the expected behaviour.
...
**Directionally accepted: C1 and C2 are accepted directions; C3 remains an
open experimental question with no selected architecture.** The claims are
separately decidable with controlled
...
2. behaviour identity and composition, instruction sources and distribution,
   selection, resolution, epistemics, authority, validity, observability, and
   protocol;
3. workflow model, local lifecycle predicates and transitions,
   mutating-command concurrency, and envelope refresh semantics;
4. activation coverage, semantic-trigger evaluation, activator deployment and
   refresh, and adherence experiments;
5. project trust establishment, hymn-cascade extensions, override integrity,
...
- PRD-006 / SPEC-009 — Current install and embedded-asset projection contract;
  revised under Contract A (REV-028).
- ADR-019 — Embedding, publication, and projection are independent asset
  policies (the seven-property vocabulary). The shared root C1 rests on.
- PRD-017 / SPEC-026 — **Contract B**: the publication, curated-library, and
  federated-search capabilities that realise C1's "semantic ownership and minimal
  projection" — governance now active (both `active`).
