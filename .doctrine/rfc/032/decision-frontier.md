...
1. **The finding has state but no history.** (`D1`) Give every finding an
   ...
   becomes derivable from authored state, the baton becomes a pure cache, and
   out-of-band edits become *detectable*.
...
3. **Ids are reserved per tree, not per clone.** (`D9`) Claim ids in the git
   common dir. That removes the collision class for every kind, and it is the
   precondition for relaxing the locus rule.
...
| D2 | `done ⇔ all terminal ∧ (non-empty ∨ concluded)`; conclude carries a basis | C1 · ISS-314 · ISS-366 · ISS-322 trap |
...
| D6 | design run binds a pass: mint by default, adopt by name | C10 · ISS-322 · C11 · C12 (part) |
| D7 | one locus predicate at the verb-family boundary; refuse only unwritable trees | C2 · C17 · F5 · ISS-484 · IMP-240 |
| D8 | roles closed + participant aliases; disposition closed-write; optional closed `route` | C13 · C14 · F4 |
| D9 | clone-wide id reservation via the git common dir; `reseat` reads leniently | C16 · ISS-279 · ISS-277 · F12 |
...
| D13 | a tech spec owns the RV mechanism; ADR-007 keeps the decision; doc drift is checked | C18 · C22 · IMP-481 · F13 |
| D14 | `RV` admitted as a relation target once D4 lands | C21 · IMP-480 · F9 |
...
better on one; those are dropped silently. Of what remains, this document picks
the option with the best *combined* position, and says what it gives up. Where
...
- Legacy findings simply have an empty journal. No migration.

**What falls out for free.**

- `rounds` becomes derivable from authored state (max turn round), so the baton
  holds **nothing non-derivable** — D-C2's "baton is a regenerable cache" becomes
  literally true. That is what makes `D7` safe.
- Out-of-band edits become detectable: for a finding with a non-empty journal,
  `status` must equal the status the last turn implies. A doctor row flags the
  mismatch (the `01a0d7f7` responder-authored `verified`, the `01a0d241`
  pre-disposed arrival). Detection, not prevention — the honest ceiling given
  `--as` is cooperative (ADR-007 Negative).
...
Why not (d): every observed incident is the *empty* case; the non-empty
ambiguity has no recorded failure, and (d)'s price is a semantic flip across the
whole corpus. The skills should still make `conclude` the normal last move of
every pass, so (d)'s coherence is approached by convention without the
migration. Revisit if a non-empty premature-`done` incident is ever recorded.
...
conclude, `ISS-322`'s alternative, is dominated: it would also refuse the honest
clean pass.
...
(c) and the conclude marker's role in it.
...
- On entry to `reviewing`, the run **binds** a pass. With no argument it mints, as
  ...
  existing RV, admitted iff it targets this slice, has facet `design`, and is
  readable in this tree. The mint remains the ordinary path; adoption is the route
  the `reviewing` obligation already advertises for an external reviewer.
...
### D7 — One locus predicate, applied once; refuse only trees that cannot write the ledger

**Question.** Where may review verbs run? (`ISS-484`, `IMP-240`, `IMP-190`,
`IMP-024`; research F5, A3.)

Today the fork refusal exists because the baton in a fork cannot be the parent's
baton. Under `D1` the baton holds nothing non-derivable, so a review worked
entirely in one tree is a single-tree review wherever that tree is — D-C7's
boundary holds per tree, and git conflicts on the authored ledger remain the
backstop, exactly as ADR-007 already says.

**Choice.**

- The predicate is evaluated **once**, in the verb-family dispatcher, for every
  verb including `new` (closing `ISS-484`: no id allocated in a tree that then
  refuses).
- It refuses only a tree that **cannot durably write** `.doctrine/review/` — a
  confined dispatch worker (the `DOCTRINE_WORKER` marker / read-only authored
  tier). Primary, coordination and solo `/worktree` trees are admitted, which is
  what `AGENTS.md`'s "audit/close on a worktree" instruction already assumes.
...
### D9 — Ids are reserved clone-wide through the git common dir
...
**Choice.** Make the **local** claim clone-wide: claim the same ref name with a
zero-oid `update-ref` CAS in the common dir, and scan those refs alongside the
local directories. No remote, no network, same namespace and scan shape as
`GitRef`, so it is a second arm of an existing backend rather than a new one.
...
Re-affirming and building the `domain_map` reader is dominated: it restores an
authoring tax whose value the one study of it (`RFC-004`) could not find.
...
- A **tech spec** owns the mechanism: schema (finding, turn, anchor, route), the
  transition graph, the derived-status function, the read projection, the locus
  predicate, conclude semantics. It is authored **in the first slice** (`D1`/`D2`/`D4`
  land there) and extended by each later slice — the RFC's "each slice leaves
  spec coverage" intent, given a concrete home.
- **No new PRD.** The product intent ("adversarial review at every lifecycle
  ...
  `IMP-481` assessment finds an existing PRD that should carry it, add it there.
...
`originates_from` included), and let `partition::status_class` read its derived
status through the engine-tier ledger module, retiring `DEC-233`'s `Unavailable`
arm for RV. Finding-granular citation (`RV-N#F-3`) is **not** introduced; prose
...
|---|---|---|---|
| 0 | *quick win (backlog item)* | D5's finding index + `list --target` over existing fields | nine incidents; no schema dependency |
| 1 | **Ledger v2** | D4 (split, with `IMP-029` golden first), D1, D2, D15, D10, D8 (write side), authors the tech spec | every later slice reads this schema; the hard, hard-to-reverse choices live here |
...
Slices 3 and 4 are independent of each other and can run in parallel.

**Governance route** (unchanged from the RFC, made concrete): slice 1's REV
amends ADR-007 D-C5 (turns, amend/reopen), D-C8 (`D2`) and, if `prime` is
touched, D-C10 (`D11`); slice 4's REV amends the D-C1/D-C7 fork clarification
(`D7`). The tech spec is authored in-slice and grows per slice.
...
- *Does a spec own the RV kind?* — yes, a tech spec, authored in slice 1 (`D13`).
  `IMP-481` narrows to confirming no PRD needs to carry product intent.
...
- **The non-empty premature-`done` ambiguity** stays (`D2`). Chosen over a
  corpus-wide semantic flip for a failure with no recorded instance.
- **Out-of-band edits are detected, not prevented** (`D1`). Nothing cooperative
  can do better; the doctor row makes them visible.
...
1. **`D2` narrow vs uniform.** If the user values one uniform rule
   (`done` always needs `conclude`) over zero migration, take option (d) and
   backfill `concluded = true` for legacy all-terminal ledgers with a
   disclosed migration note. Recommendation: narrow.
...
- New: amend/reopen acts (partly `ISS-485`), conclude `--basis`, the install-doc
  flag conformance doctor check, clone-wide local reservation, repo-wide secret
  scanning, the quick-win finding index (split out of `IMP-475`).
- Narrow: `IMP-481` (to "no PRD needed?"), `IMP-392` (to D3 + D6).
- Close on landing: `ISS-314`, `ISS-366` (duplicate), `IMP-479` (not needed),
  `IMP-478` (by REV).
- Retract: `ISS-314 needs IMP-392` once `CHR-057` gives a verb for it.
