...
[backlog + link; mcp-dispatch-idea-session]
- `doctrine memory retrieve <key>` rejects a positional key ("unexpected argument"); retrieve is scope/query-flag driven, not id-addressed. `show`/`find` take the id. Cost me one failed call reaching for a memory by key from the boot index.
- `doctrine link IDE-027 references RFC-011 --role originates_from` rejected: references target must be one of [ISS,IMP,CHR,RSK,IDE,SL] — an RFC is not a legal `references` target. Backlog provenance from an RFC has no typed edge; fell back to `related`. If RFC→backlog provenance is meaningful (RFC-011 case notes literally originated this idea), `originates_from` can't express it.
