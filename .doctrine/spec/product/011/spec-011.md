# PRD-011: Graph-Derived Backlog Priority

## 1. Intent

Doctrine captures work before it is scoped (PRD-009): an issue, improvement, chore,
risk, or idea lands the moment it surfaces, then is triaged, resolved, or promoted into
a slice. That capture layer is deliberately cheap. But a cheap capture layer leaves a
hard question unanswered — *what should I look at next, and why?* — and a flat priority
field cannot answer it for a governed corpus. Some work is urgent because an operator
ranked it; some because it blocks a slice, satisfies a requirement, mitigates an active
risk, answers an open question, or clears drift; some is not actionable yet because a
predecessor is unresolved or a governance question is still open. Authored priority
alone collapses all of that into one hand-set number that rots the moment the graph
around it moves.

This capability adds a small, native, **graph-derived** priority and actionability layer
over doctrine's existing entity graph. It keeps authored backlog priority intentionally
small (PRD-009 owns that seam), derives richer ordering from the typed relations already
in the corpus, and produces **inspectable explanations** without ever storing derived
judgement as backlog truth. Its value is that prioritisation becomes *defensible without
becoming heavy*: capture stays cheap, yet survey and "next work" views account for
blockers, dependencies, lifecycle state, explicit ordering, and governance pressure, and
a reviewer can always see *why* an item ranked where it did. Doctrine gains a native
...
- A doctrine policy mapping existing entity kind, lifecycle, resolution, and relation
  ...
  reason.
- Registry-backed survey / next-work / inspect / explain / blockers surfaces that show
  derived inbound references, blocking state, and priority explanations.
...
Boundary: this capability owns **derived graph interpretation for priority-like backlog
surfaces** — the read-side context computed over the existing corpus. It does not own
work capture, item kind membership, item lifecycle, slice execution, scheduling, or
product-specific urgency philosophy. The durable backlog item remains the source of
captured work intent; the technical shape of the graph crate, its module boundary, and
the v1 policy's channel and relation-kind layout are downstream `/spec-tech` concerns,
not product intent.
...
- **The richer-engine seam is intentional.** The graph core and doctrine policy are
  ...
  channels without changing authored backlog storage.
...
- A default backlog survey must remain useful without the derived engine — the derived
  surface enhances survey/next/explain/blockers behaviour; it is not required for
  capture.

Invariants:

- A backlog item's lifecycle remains authoritative on the item. Derived actionability
  never mutates `status`, `resolution`, `item_kind`, priority metadata, or relations.
...
- A terminal backlog item is hidden from default active derived output unless explicitly
  included; a promoted item is not treated as active work intake — the promoted slice is
  authoritative for the scoped work.
...
- A backlog item carrying explicit authored priority orders ahead of its unranked peers
  in derived survey.
- A derived priority query excludes terminal and promoted items by default and includes
...
Primary flow — derived survey: an operator asks for the backlog with derived priority.
The system scans doctrine entities into the registry graph, applies the doctrine policy,
and returns active backlog items ordered by, in policy-defined precedence: explicit
authored priority where present; actionability state; relation-derived consequence or
pressure; kind/status policy; and a stable deterministic fallback. The exact weights are
...
Edge cases and guards: a backlog item with no authored priority is normal and orders by
derived context plus deterministic fallback; a terminal or promoted item stays
addressable but is excluded from default active output; a derived answer with no admitted
relation evidence still carries a lifecycle-and-fallback explanation rather than an empty
one.

## 7. Verification

Verification confirms that derived priority is deterministic, explainable, graph-backed,
and strictly separate from authored backlog truth — without binding the spec to a
particular implementation.

Graph construction is proven by seeding a registry with backlog items and related
artefacts and confirming the adapter emits the expected nodes and typed edges while
...
inbound field. Actionability is proven by confirming an item with an unresolved admitted
blocker is marked non-actionable while an otherwise-equivalent unblocked item stays
actionable. Ordering is proven by confirming explicit priority, actionability,
