# Skill content refresh = doctrine skills install + touch src/skills.rs to re-embed
...
- **`doctrine install` does NOT refresh skill content.** It only ensures the
  ...
  is `doctrine skills install -y` (filter with `-s <id>` / `-d <domain>`).
...
./target/debug/doctrine skills install -y   # refresh .doctrine/skills/* + relink .claude/skills/*
```
