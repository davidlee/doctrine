# DEC prefix dual-namespaced — numbered kind (2-part) vs external forgettable cites (3-part)
...
- **`DEC-NNN-XX` (3-part, letter/number suffix: `DEC-005-C`, `DEC-010-06`)** —
  **external forgettable decision-log citations**, sprinkled through SL-007/012
  prose (forgettable byte-reproduction work), `src/git.rs` comments, memory, and
  the boot snapshot. Free-text prose, **never doctrine entities, never
  renumbered** (provenance to forgettable's own log). Doctrine's *own* doc-local
...
(recommended — external forgettable refs survive) vs validate numbered DEC. Do
**not** sed-rename the 3-part external citations. See [[mem.fact.backend.forgettable-event-store]]
