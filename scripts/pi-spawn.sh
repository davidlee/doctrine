...
# Negative pid = signal the whole group. The chain is timeout -> env -C -> pi, so
# $! is the TIMEOUT; a bare kill fells only that and orphans the real pi holding
# its API session — and a background `wait` on this script then blocks until the
# orphan dies. Observed: two orphans alive 16 minutes after writing output.
...
# silent full-backstop burn — the commit lands either way (ISS-293).
