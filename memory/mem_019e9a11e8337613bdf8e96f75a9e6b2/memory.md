...
1. **/route** — the mandatory gate. Picks the governing skill before you touch
   anything.
2. **slice** — scope the change into `slice-nnn.{toml,md}` under
   `.doctrine/slice/nnn/`. The CLI is the source of truth:
   `doctrine slice --help`.
3. **design** — author `design.md`, then adversarial review (`/inquisition`)
   until the decisions lock.
4. **plan** — `plan.toml` (phases + EN/EX/VT criteria) + `plan.md`
   (rationale). Then materialise the runtime tracking sheets.
