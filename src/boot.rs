...
#[cfg_attr(not(test), expect(dead_code, reason = "retained for future non-pi harness use"))]
enum SymlinkAction {
...
function resolveBoot(): string {{
  try {{
    return execSync("{exec_str} boot --emit", {{
      encoding: "utf-8",
      cwd: process.cwd(),
      timeout: 5_000,
    }}).trimEnd();
    ...
    for h in harnesses {
        match install_refresh(h, root, exec, dry_run) {
            Ok(report) => {
                ...
                // session_start extension that resolves `doctrine boot --emit`
                // and injects the output via before_agent_start. Claude
                // carries NotApplicable and stays silent.
...
"#
    ...
    fn install_pi_extension_generates_file() {
        ...
        assert!(generated.contains("/abs/doctrine boot --emit"));
