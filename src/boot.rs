    ...
    for h in harnesses {
        match install_refresh(h, root, exec, dry_run) {
            Ok(report) => {
                ...
                // session_start extension that resolves `doctrine prompt resolve
                // --role orchestrator` (SL-187 PHASE-04, CHR-033 seam) and injects
                // the output via before_agent_start. Claude carries NotApplicable
                // and stays silent.
