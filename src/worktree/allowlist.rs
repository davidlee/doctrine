...
pub(crate) enum Tier {
    ...
    Dispatch,
    /// `.doctrine/observations/**/.tmp.*` — a reserved publication temporary
    /// (SL-231). The observation writer publishes by hard-linking a complete
    /// sibling temp into place, so one of these exists only where a capture was
    /// interrupted. It is a partial artefact of ONE tree's crash: meaningless
    /// anywhere else, never authored, and never regenerated. The records
    /// themselves are authored and deliberately NOT withheld.
    ObservationTemp,
}
...
impl std::fmt::Display for Tier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let name = match self {
            ...
            Tier::ObservationTemp => "observation-temp",
        };
        f.write_str(name)
    }
}
...
pub(crate) const WITHHELD: &[Withhold] = &[
    ...
    w(Tier::ObservationTemp, ".doctrine/observations/**/.tmp.*"),
];
...
mod tests {
    ...
    /// SL-231 PHASE-05: the reserved publication temporary is withheld, and the
    /// authored records beside it are NOT. Getting this wrong in the withhold
    /// direction is invisible — a fork would silently lose the corpus — so the
    /// negative half carries the weight here.
    #[test]
    fn observation_temps_are_withheld_but_records_are_not() {
        assert_eq!(
            is_withheld(".doctrine/observations/records/ab/.tmp.9182.0.pub"),
            Some(Tier::ObservationTemp)
        );
        assert_eq!(
            is_withheld(
                ".doctrine/observations/records/ab/019f1234-5678-7abc-8def-0123456789ab.toml"
            ),
            None,
            "authored records must reach a fork — they are committed collection data"
        );
    }
