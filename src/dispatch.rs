...
fn rung_detail(
    ...
    match kind {
        NextKind::Spawn => format!(
            "{p} has no funnel row yet — spawn its worker: `worktree fork --worker` at \
              the phase base, then the confined spawn script. No command literal, because \
              the spawn is a script invocation carrying the phase prompt rather than a \
              single doctrine verb. Hand off to the spawn skill."
        ),
...
mod tests {
    ...
    mod next_oracle {
        ...
        fn spawn_takes_the_first_ready_phase_with_no_funnel_row() {
            ...
            assert_eq!(core.phase.as_deref(), Some("PHASE-02"));
            // SL-254 PHASE-06: the rung carried no command literal because the spawn
            // was ARM-ROUTED and the oracle stayed arm-agnostic (D-P6-5). One arm
            // survives, so the reason changed — the spawn is a SCRIPT invocation
            // carrying the phase prompt, not a single doctrine verb — but the property
            // (no literal; hand off to the spawn skill) is the same and still pinned.
            assert_eq!(
                core.command, None,
                "spawn carries no command literal — it is a script invocation (D-P6-5)"
            );
            assert!(
                core.detail.contains("spawn skill"),
                "spawn hands off to the spawn skill: {}",
                core.detail
            );
        }
