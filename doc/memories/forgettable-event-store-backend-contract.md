# Memory: the external decision register is the event-store backend (DEC-005-C constraint)

The deferred `event-store` backend ([memory-spec § Backend abstraction](../memory-spec.md))
is **the external decision register**. It accepted the client role in the external decision register `ADR-005`
...
The doctrine adapter **MUST NOT depend on the external decision register's first-party `/memory/*`
...
fidelity never relies on the external decision register's own git-scoped memory feature. Calling
...
## Stream shape the external decision register binds (for the adapter slice)
...
## Read surfaces the external decision register commits to — DEC-005-D

the external decision register un-defers its "global commit-safe cursor" against doctrine as first
...
## Locating the external decision register

- agent jails (most agents): `/workspace/the external decision register`
- the ADR: `<the external decision register>/.spec-driver/decisions/ADR-005-*.md`
