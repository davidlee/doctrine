    ...
    flake-parts.lib.mkFlake {inherit inputs;} {
      ...
      perSystem = {system, ...}: let
        ...
        srcWithDist = pkgs.runCommandLocal "doctrine-src" {} ''
          cp -R ${cleanedSrc} $out
          chmod -R u+w $out
          rm -rf $out/plugins $out/install $out/memory $out/templates $out/agentslication
          mkdir -p $out/plugins $out/install $out/memory $out/web/map/dist $out/templates $out/agentslication
          cp -R ${./plugins}/.     $out/plugins/
          cp -R ${./install}/.     $out/install/
          cp -R ${./memory}/.      $out/memory/
          cp -R ${./templates}/.   $out/templates/
          cp -R ${./agentslication}/. $out/agentslication/
      ...
      in {
        ...
        packages =
          ...
          // {
            inherit doctrine;
            # Unjailed dirge pulled straight from the agents flake — same pkgs +
            ...
            # Must come from agents's eval; doctrine's own pkgs (different nixpkgs +
            # rust-overlay pin) would fork the drv and rebuild from scratch.
            dirge = inputs.agents.packages.${system}.dirge;
        ...
        devshells.default = {
          packages =
            ...
            # set. From agents's eval (jailLib.unjailed) so they're the identical
